...
const FlowerSVG: React.FC<FlowerSVGProps> = ({ type, color, size = 60, className = '', onClick }) => {
  ...
  const id = `flower-${type}-${color.replace('#', '')}-${Math.random().toString(36).substr(2, 4)}`;
  ...
  switch (type) {
    ...
    case 'lavender':
      return (
        <svg width={size} height={size * 1.5} viewBox="0 0 60 90" className={className} onClick={onClick} style={{ cursor: onClick ? 'pointer' : 'default' }}>
          <defs>
            <linearGradient id={`${id}-lav`} x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={light} />
              <stop offset="100%" stopColor={color} />
            </linearGradient>
          </defs>
          {/* Main stem */}
          <path d="M30 35 Q29 55 30 90" stroke={stemColor} strokeWidth="2" fill="none" strokeLinecap="round" />
          {/* Side stems */}
          <path d="M30 50 Q24 44 22 48" stroke={stemLight} strokeWidth="1" fill="none" />
          <path d="M30 60 Q36 54 38 58" stroke={stemLight} strokeWidth="1" fill="none" />
          {/* Lavender flower buds - clustered along stem */}
          {[
            { cx: 30, cy: 6, r: 3.5 },
            { cx: 27, cy: 10, r: 3 },
            { cx: 33, cy: 10, r: 3 },
          ...
          {/* Subtle highlight */}
          <ellipse cx="30" cy="16" rx="6" ry="14" fill={light} opacity="0.15" />
        </svg>
      );
    default:
      return null;
